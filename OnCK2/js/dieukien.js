function checkSoSerial() {
    var so = document.getElementById('txt_serial').value;
    var check_so = document.getElementById('error_serial');
...
function checkTrongLuong() {
    ...
    if (trongluong == '' || trongluong == null)
        ...
        check_kg.innerHTML = 'đây không phải là số';
    else if (trongluong <= 0)
        check_kg.innerHTML = 'số phải lớn hơn 0';
    else {
        check_kg.innerHTML = '';
        return trongluong;
    }
...
let stt = 1;
function dathang() {
    let so = document.getElementById('txt_serial').value;
